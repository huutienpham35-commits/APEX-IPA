...
struct FeatureVisibility: Equatable {
    ...
    let gamesEnabled: Bool
    let patchesEnabled: Bool
    let wallpaperEnabled: Bool
    let filesEnabled: Bool

    init(cleanerEnabled: Bool, homeEnabled: Bool = true, gamesEnabled: Bool = true, patchesEnabled: Bool = true, wallpaperEnabled: Bool = true, filesEnabled: Bool = true) {
        self.cleanerEnabled = cleanerEnabled
        self.homeEnabled = homeEnabled
        self.gamesEnabled = gamesEnabled
    ...
    var visibleSections: [AppSection] {
        let enabled = AppSection.allCases.filter(isVisible)
    ...
    func isVisible(_ section: AppSection) -> Bool {
        ...
        case .home: return homeEnabled
        case .games: return gamesEnabled
        case .patches: return patchesEnabled
        case .wallpaper: return wallpaperEnabled
        case .files: return filesEnabled
        }
    }
}
...
struct AppTabNavigationState: Equatable {
    ...
    mutating func reconcileSelection(with visibility: FeatureVisibility) {
        guard let selectedSection = AppSection(rawValue: selectedTab),
            ...
            selectedTab = visibility.visibleSections.first?.rawValue ?? AppSection.home.rawValue
            return
        }
    }
}
